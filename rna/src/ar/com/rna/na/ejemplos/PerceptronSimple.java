package ar.com.rna.na.ejemplos;
...
public class PerceptronSimple implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == entrenar) {
			if (establecioSalidas == 0) {
				...
				int contador = 0;
				/*
				 * recorremos las entradas y se le van pasando a la
				 * funcionActivacion() esta funcion nos regresa la salida para
				 * dichas entradas. por ejemplo si elegimos la compuerta OR y se
				 * le manda la entradas: x1 = 1 , x2 = 0 la salida que nos
				 * deberia de regresa la funcionActivacion() es 1 esta salida se
				 * le manda al metodo error este verifica si hay o no error
				 */
			...
			// ya una ves que el perceptron este entrenado podemos dar clic en
			// el boton probar y nos ira pidiendo las entradas
			...
			String x1 = JOptionPane.showInputDialog(null,
					"Ingresa la primera entrada");
			String x2 = JOptionPane.showInputDialog(null,
					"Ingresa la segunda entrada");
	...
	public void calculaPesos(float[] entradas, float salidas) {
		if (error != 0) {
			for (int i = 0; i < entradas.length; i++) {
				System.out.println(pesos[i] + " + (2 * .5) * " + salidas
						+ " * " + entradas[i]);
				this.pesos[i] = pesos[i] + (2.0f * .5f)
						* (salidas * entradas[i]);
				...
				System.out.println("AHORA LOS PESOS CAMBIARON A :"
						+ this.pesos[i]);
			}
		}
	}
